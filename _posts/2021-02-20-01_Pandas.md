...
**Pandas**的核心数据分析支持库，提供了快速、灵活、明确的数据结构，旨在简单、直观地处理关系型、标记型数据。Pandas 的目标是成为 Python 数据分析实践与实战的必备高级工具，其长远目标是成为**最强大、最灵活、可以支持任何语言的开源数据分析工具**。经过多年不懈的努力，Pandas 离这个目标已经越来越近了。

<!-- more -->

转自：https://www.pypandas.cn/docs/getting_started/10min.html
...
用值列表生成 [Series (opens new window)](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.html#pandas.Series)时，Pandas 默认自动生成整数索引：
...
> [DataFrame.to_numpy() (opens new window)](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.to_numpy.html#pandas.DataFrame.to_numpy)的输出不包含行索引和列标签。
...
Series 的 `str` 属性包含一组字符串处理功能，如下列代码所示。注意，`str` 的模式匹配默认使用[正则表达式](https://docs.python.org/3/library/re.html)。
...
In [96]: index = pd.MultiIndex.from_tuples(tuples, names=['first', 'second']) # 多列索引
...
![可视化](https://gitee.com/zhiliangj/Typora_Img/raw/master/series_plot_basic.png)
...
![可视化2](https://gitee.com/zhiliangj/Typora_Img/raw/master/frame_plot_basic.png)
